...
export default class Bow {
  constructor(appear, stay, disappear, ap0, ap1, lineWidth, strokeStyle, shifting, upper) {
    this.appear = appear;
    this.stay = stay;
  ...
  drawArcs(ctx, start, end, lineWidth) {
    ...
    ratios.forEach((ratio) => {
      ...
      ctx.quadraticCurveTo((1 - ratio) * start.x + ratio * end.x, ratio * start.y + (1 - ratio) * end.y, end.x, end.y);
      ctx.stroke();
    });
  }
}
